...
constructor(@param:Named("java") private val commonCodeStyleSettings: CommonCodeStyleSettings) : SettingsApplier<JavaKeepMaximumBlankLinesSettings> {
    ...
    override fun apply(settings: JavaKeepMaximumBlankLinesSettings) {
        settings.inDeclarations()
            .ifPresent { commonCodeStyleSettings.KEEP_BLANK_LINES_IN_DECLARATIONS = it }
        settings.inCode()
            .ifPresent { commonCodeStyleSettings.KEEP_BLANK_LINES_IN_CODE = it }
        settings.beforeRightBrace()
            .ifPresent { commonCodeStyleSettings.KEEP_BLANK_LINES_BEFORE_RBRACE = it }
        settings.betweenHeaderAndPackage()
            .ifPresent { commonCodeStyleSettings.KEEP_BLANK_LINES_BETWEEN_PACKAGE_DECLARATION_AND_HEADER = it }
    }
}
